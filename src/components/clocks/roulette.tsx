...
export default function ClockRoulette({ className }: { className?: string }) {
  ...
  React.useEffect(() => {
    ...
    const updateClock = () => {
      ...
      if (secHandRef.current) {
        secHandRef.current.style.transform = `translate(-50%, -50%) rotate(${finalSecDeg}deg) translateY(-${RADIUS}px)`;
      }
      if (minHandRef.current) {
        minHandRef.current.style.transform = `translate(-50%, -50%) rotate(${finalMinDeg}deg) translateY(-${RADIUS}px)`;
      }
      if (hourHandRef.current) {
        hourHandRef.current.style.transform = `translate(-50%, -50%) rotate(${finalHourDeg}deg) translateY(-${RADIUS}px)`;
      }
  ...
  const ticks = React.useMemo(() => {
    return Array.from({ length: 12 }).map((_, i) => {
      ...
      return (
        <div
          key={i}
          // h-3.5 (14px) 略小于边框宽度(15px)，确保完全在环内
  ...
  if (!mounted) {
    return (
      <div className={cn("relative mx-auto h-[300px] w-[300px] flex items-center justify-center bg-card rounded-full", className)}>
        <div className="h-full w-full rounded-full border-[15px] border-muted/10" />
      </div>
    );
  }
  ...
  return (
    ...
    >
      {/* 光环容器 (圆环背景) */}
      <div className="relative box-border flex h-full w-full items-center justify-center rounded-full border-[15px] border-foreground/10 bg-transparent">
