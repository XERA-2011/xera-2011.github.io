...
export default function ClockSteampunk({ className }: { className?: string }) {
  ...
  if (!mounted) return (
    <div className={cn("relative flex items-center justify-center", className)} style={{ width: 300, height: 300 }}>
      <div className="w-full h-full rounded-full bg-[#1a120b]" />
    </div>
  );
  ...
  // 缩放比例: 300 / 460 ≈ 0.65217
  const SCALE = TARGET_SIZE / ORIGINAL_SIZE;

  return (
    <div
      className={cn(
        "relative flex items-center justify-center font-serif overflow-hidden select-none",
        cinzel.className,
        className
      )}
      style={{
    ...
    >
      {/* 
        核心修复：
        使用 transform: scale() 整体缩放 460px 的内容到 300px。
        transformOrigin: "center" 确保居中缩放。
      */}
      <div
        style={{
          width: ORIGINAL_SIZE,
        ...
        className="relative"
      >
        {/* 修改：移除了 shadow-[0_30px_60px_...] 及其它阴影 */}
