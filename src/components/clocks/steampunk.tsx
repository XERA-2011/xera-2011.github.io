...
interface ClockProps {
  className?: string;
  /** 
   * 如果传入此参数，时钟将变为【静态模式】，不会运行动画，不仅节省性能，还能展示指定时间。
...
export default function ClockSteampunk({ className, staticTime }: ClockProps) {
  ...
  }, [staticTime]); // 依赖 staticTime，切换模式时会重置

  // --- 静态资源 (ticks, numerals, gears) ---
  // (这部分代码保持不变，省略 memo 以节省篇幅，实际使用请保持原有的 useMemo)
  ...
  const numerals = ROMANS.map((num, i) => {
  ...
  const gears = React.useMemo(() => {
    ...
    // 简化：这里为了合并代码，路径生成逻辑稍微硬编码了一些，确保和之前一致即可
  ...
  // 辅助：获取样式，如果是静态模式返回 transform，否则返回 undefined (交给 Ref 控制)
    ...
    <div className={cn("relative flex items-center justify-center font-serif overflow-hidden select-none", cinzel.className, className)}
      ...
      <div style={{ width: ORIGINAL_SIZE, height: ORIGINAL_SIZE, transform: `scale(${SCALE})`, transformOrigin: "center", flexShrink: 0 }} className="relative">
        <div className="relative w-[460px] h-[460px] rounded-full">
          {/* 金属外壳 */}
          <div className="absolute inset-0 rounded-full z-1 shadow-[inset_0_0_10px_rgba(0,0,0,0.8),0_0_0_2px_#221,0_0_0_12px_#150f08]"
