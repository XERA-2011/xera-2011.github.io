...
export function MarkdownPage({
  ...
  const defaultComponents: Components = {
    ...
    p: ({ ...props }) => <p className="text-base leading-7 mb-2 text-white/70" {...props} />,
    ...
    code: (props) => {
      ...
      return inline ? (
        <code className="bg-theme-black/50 px-1.5 py-0.5 rounded text-sm text-green-400" {...rest} />
      ) : (
        <code className="block bg-theme-black/50 p-3 rounded-lg overflow-x-auto text-sm text-green-400" {...rest} />
      );
    },
    pre: ({ ...props }) => (
      <pre className="bg-theme-black/50 rounded-lg overflow-x-auto my-4 -mx-2 sm:mx-0" {...props} />
    ),
    // 块引用样式
    blockquote: ({ ...props }) => (
