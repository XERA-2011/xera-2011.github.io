...
export default function GitHubSnakePage() {
  ...
  const snakeDark = `/api/redirect?url=${encodeURIComponent(`${cdnBase}/snake-dark.svg`)}`;
  const snakeLight = `/api/redirect?url=${encodeURIComponent(`${cdnBase}/snake-light.svg`)}`;

  const currentSrc = isDark ? snakeDark : snakeLight;
          ...
          <Card className="bg-card/50 backdrop-blur-sm border-border/50">
            ...
            <CardContent className="space-y-8">
              <div className="space-y-4">
                ...
                <div className={`relative w-full aspect-5/2 rounded-lg overflow-hidden border ${isDark ? 'bg-[#0d1117] border-[#30363d]' : 'bg-white border-[#e1e4e8]'}`}>
                  <Image
                    key={currentTheme}
                    src={currentSrc}
                    alt={`GitHub Snake ${isDark ? 'Dark' : 'Light'}`}
                    fill
