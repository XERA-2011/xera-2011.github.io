...
// 缓存时间（秒）- 增加缓存时间以减少 API 请求频率
const CACHE_SECONDS = 60; // 从 30 秒增加到 60 秒
...
const CACHE_DURATION = CACHE_SECONDS * 1000; // 毫秒
 ...
 *
 * 示例:
 * - /api/coin-card?coin=btc
 * - /api/coin-card?coin=btc,eth,sol,bnb&mode=multi
 */
  ...
  try {
    ...
    if (cachedEntry && (now - cachedEntry.timestamp) < CACHE_DURATION) {
      ...
      const response = await fetch(apiUrl, {
        ...
        next: { revalidate: CACHE_SECONDS }, // Next.js 缓存
      });
